//
//  HPanModalPresentationController.swift
//  PanModal
//
...
//
     ...
     */
    open class HPanModalPresentationController: UIPresentationController {
         ...
         */
        private var presentable: HPanModalPresentable? {
            return presentedViewController as? HPanModalPresentable
        }
         ...
         */
        private lazy var backgroundView: HDimmedView = {
            let view: HDimmedView
            if let color = presentable?.panModalBackgroundColor {
                view = HDimmedView(dimColor: color)
            } else {
                view = HDimmedView()
            }
         ...
         */
        private lazy var panContainerView: HPanContainerView = {
            let frame = containerView?.frame ?? .zero
            return HPanContainerView(
    ...
    public extension HPanModalPresentationController {
    ...
    private extension HPanModalPresentationController {
        ...
        func addDragIndicatorView(to view: UIView) {
            ...
            dragIndicatorView.bottomAnchor
                .constraint(
        ...
        func configureViewLayout() {
            ...
            guard let layoutPresentable = presentedViewController as? HPanModalPresentable.LayoutType
            else { return }
    ...
    private extension HPanModalPresentationController {
        ...
        func snap(toYPosition yPos: CGFloat) {
            HPanModalAnimator.animate({ [weak self] in
    ...
    private extension HPanModalPresentationController {
        ...
        func haltScrolling(_ scrollView: UIScrollView) {
            scrollView.setContentOffset(
                CGPoint(x: 0, y: scrollViewYOffset),
                animated: false
            )
            scrollView.showsVerticalScrollIndicator = false
        }
    ...
    extension HPanModalPresentationController: UIGestureRecognizerDelegate {
    ...
    private extension HPanModalPresentationController {
        ...
        func addRoundedCorners(to view: UIView) {
            ...
            if presentable?.showDragIndicator == true {
                ...
                drawAroundDragIndicator(
                    currentPath: path,
                    indicatorLeftEdgeXPos: indicatorLeftEdgeXPos
                )
            }
         ...
         */
        func drawAroundDragIndicator(
            currentPath path: UIBezierPath,
            indicatorLeftEdgeXPos: CGFloat
        ) {
            ...
            path.addLine(to: CGPoint(
                x: indicatorLeftEdgeXPos,
                y: path.currentPoint.y
            ))
            path.addLine(to: CGPoint(
                x: path.currentPoint.x,
                y: path.currentPoint.y - totalIndicatorOffset
            ))
            path.addLine(to: CGPoint(
